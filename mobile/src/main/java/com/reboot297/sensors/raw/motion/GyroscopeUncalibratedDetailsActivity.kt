...
import com.reboot297.sensors.databinding.ActivityDetailsGyroscopeUncalibratedBinding

class GyroscopeUncalibratedDetailsActivity : BaseActivity(), SensorEventListener {

    private lateinit var sensorManager: SensorManager
    private var _sensor: Sensor? = null
    private val sensor: Sensor? get() = _sensor
    private lateinit var binding: ActivityDetailsGyroscopeUncalibratedBinding


    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)

        binding = ActivityDetailsGyroscopeUncalibratedBinding.inflate(layoutInflater)
    ...
    private fun startListening() {
        sensorManager.registerListener(
            this@GyroscopeUncalibratedDetailsActivity,
            sensor,
    ...
    private fun displaySensorInfo(sensor: Sensor) = with(binding) {
        ...
        sensorMaxRangeValue.text = getString(R.string.format_unit_gyroscope, sensor.maximumRange)
        sensorResolutionValue.text = getString(R.string.format_unit_gyroscope, sensor.resolution)
        ...
        sensorUnitValue.text = getString(R.string.unit_gyroscope)
    ...
    override fun onSensorChanged(event: SensorEvent?) {
        event?.values?.let {
            binding.sensorValueRawXView.text = getString(R.string.format_unit_gyroscope, it[0].toString())
            binding.sensorValueRawYView.text = getString(R.string.format_unit_gyroscope, it[1].toString())
            binding.sensorValueRawZView.text = getString(R.string.format_unit_gyroscope, it[2].toString())
            binding.sensorValueXView.text = getString(R.string.format_unit_gyroscope, it[3].toString())
            binding.sensorValueYView.text = getString(R.string.format_unit_gyroscope, it[4].toString())
            binding.sensorValueZView.text = getString(R.string.format_unit_gyroscope, it[5].toString())
        }
    }
