...
import com.reboot297.sensors.databinding.ActivityDetailsGravityBinding

class GravityDetailsActivity : BaseActivity(), SensorEventListener {

    private lateinit var sensorManager: SensorManager
    private var _sensor: Sensor? = null
    private val sensor: Sensor? get() = _sensor
    private lateinit var binding: ActivityDetailsGravityBinding


    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)

        binding = ActivityDetailsGravityBinding.inflate(layoutInflater)
    ...
    private fun displaySensorInfo(sensor: Sensor) = with(binding) {
        ...
        sensorMaxRangeValue.text = getString(R.string.format_unit_gravity, sensor.maximumRange)
        sensorResolutionValue.text = getString(R.string.format_unit_gravity, sensor.resolution)
        ...
        sensorUnitValue.text = getString(R.string.unit_gravity)
    ...
    override fun onSensorChanged(event: SensorEvent?) {
        event?.values?.let {
            binding.sensorValueXView.text = getString(R.string.format_unit_gravity, it[0].toString())
            binding.sensorValueYView.text = getString(R.string.format_unit_gravity, it[1].toString())
            binding.sensorValueZView.text = getString(R.string.format_unit_gravity, it[2].toString())
        }
    }
