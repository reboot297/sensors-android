...
import com.reboot297.sensors.databinding.ActivityDetailsStepCounterBinding

class StepCounterDetailsActivity : BaseActivity(), SensorEventListener {

    private lateinit var sensorManager: SensorManager
    private var _sensor: Sensor? = null
    private val sensor: Sensor? get() = _sensor
    private lateinit var binding: ActivityDetailsStepCounterBinding


    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)

        binding = ActivityDetailsStepCounterBinding.inflate(layoutInflater)
    ...
    private fun displaySensorInfo(sensor: Sensor) = with(binding) {
        ...
        sensorMaxRangeValue.text = getString(R.string.format_unit_step_counter, sensor.maximumRange)//TODO(Viktor) use plurals
        sensorResolutionValue.text = getString(R.string.format_unit_step_counter, sensor.resolution)
        ...
        sensorUnitValue.text = getString(R.string.unit_step)
    ...
    override fun onSensorChanged(event: SensorEvent?) {
        event?.values?.firstOrNull()?.let {
            binding.sensorValueView.text = getString(R.string.format_unit_step_counter, it.toString())
        }
    }
