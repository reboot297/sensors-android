...
import com.reboot297.sensors.databinding.ActivityDetailsRotationVectorBinding

class RotationVectorDetailsActivity : BaseActivity(), SensorEventListener {

    private lateinit var sensorManager: SensorManager
    private var _sensor: Sensor? = null
    private val sensor: Sensor? get() = _sensor
    private lateinit var binding: ActivityDetailsRotationVectorBinding


    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)

        binding = ActivityDetailsRotationVectorBinding.inflate(layoutInflater)
    ...
    private fun startListening() {
        sensorManager.registerListener(
            this@RotationVectorDetailsActivity,
            sensor,
    ...
    private fun displaySensorInfo(sensor: Sensor) = with(binding) {
        ...
        sensorMaxRangeValue.text = sensor.maximumRange.toString()
        sensorResolutionValue.text = sensor.resolution.toString()
        ...
        sensorUnitValue.text = getString(R.string.unitless)
    ...
    override fun onSensorChanged(event: SensorEvent?) {
        event?.values?.let {
            binding.sensorValueXView.text = it[0].toString()
            binding.sensorValueYView.text = it[1].toString()
            binding.sensorValueZView.text = it[2].toString()
            binding.sensorValueScalarView.text = it[3].toString()
        }
    }
