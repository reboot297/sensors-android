...
import com.reboot297.sensors.databinding.ActivityDetailsGyroscopeBinding

class GyroscopeDetailsActivity : BaseActivity(), SensorEventListener {

    private lateinit var sensorManager: SensorManager
    private var _sensor: Sensor? = null
    private val sensor: Sensor? get() = _sensor
    private lateinit var binding: ActivityDetailsGyroscopeBinding


    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)

        binding = ActivityDetailsGyroscopeBinding.inflate(layoutInflater)
    ...
    override fun onSensorChanged(event: SensorEvent?) {
        event?.values?.let {
            binding.sensorValueXView.text = getString(R.string.format_unit_gyroscope, it[0].toString())
            binding.sensorValueYView.text = getString(R.string.format_unit_gyroscope, it[1].toString())
            binding.sensorValueZView.text = getString(R.string.format_unit_gyroscope, it[2].toString())
        }
    }
