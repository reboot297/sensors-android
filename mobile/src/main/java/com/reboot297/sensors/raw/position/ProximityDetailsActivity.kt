...
import com.reboot297.sensors.databinding.ActivityDetailsProximityBinding

class ProximityDetailsActivity : BaseActivity(), SensorEventListener {

    private lateinit var sensorManager: SensorManager
    private var _sensor: Sensor? = null
    private val sensor: Sensor? get() = _sensor
    private lateinit var binding: ActivityDetailsProximityBinding


    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)

        binding = ActivityDetailsProximityBinding.inflate(layoutInflater)
    ...
    private fun displaySensorInfo(sensor: Sensor) = with(binding) {
        ...
        sensorMaxRangeValue.text = getString(R.string.format_unit_proximity, sensor.maximumRange)
        sensorResolutionValue.text = getString(R.string.format_unit_proximity, sensor.resolution)
        ...
        sensorUnitValue.text = getString(R.string.unit_proximity)
    ...
    override fun onSensorChanged(event: SensorEvent?) {
        event?.values?.firstOrNull()?.let {
            binding.sensorValueView.text = getString(R.string.format_unit_proximity, it.toString())
        }
    }
